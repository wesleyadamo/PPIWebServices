...
public class CarrosDao {
	...
	public boolean atualizarCarro(Carro carro) {
		PreparedStatement stmt;
		try {
			stmt = this.connection.prepareStatement("UPDATE carros"
					+ " SET modelo = ?, categoria= ?, anoFabricacao= ? , tarifaDia = ? " + " WHERE renavan = ? ;");
	...
	public void removerCarro(long renavan) {
		try {
			...
			PreparedStatement stmt = this.connection.prepareStatement(
					"Delete from carros where renavan = ? AND renavan NOT IN (SELECT renavanDoCarro FROM aluguel WHERE renavanDoCarro = ?)");
			stmt.setLong(1, renavan);
	...
	public List<Carro> getCarrosDisponiveis() {
		...
		try {
			...
			PreparedStatement stmt = this.connection.prepareStatement(
					"select * from carros where renavan  not in (select renavanDoCarro from aluguel where  dataFinalAluguel > ? );");
	...
	public List<Carro> getCarrosDisponiveis2(){
	...
	public List<Carro> getCarrosTipo(String tipo){
		
		List<Carro> carros = new ArrayList<Carro>();
		
		try {
			
			PreparedStatement stmt = this.connection.prepareStatement("select * from carros where categoria = ?");
